...
std::pair<double , double> getMean(std::vector<double> data){
    ...
    if(data.empty()) return std::make_pair(0., 0.);
...
std::pair<EDouble, EDouble> analyse(std::string file2analyse, const config& c)
{
    ...
    TTree* inputTree;
    if(c.simulation) inputTree = dynamic_cast<TTree*>( inputFile.Get("layer_0") );
    else inputTree = dynamic_cast<TTree*>( inputFile.Get("rawData") );
    ...
    else
    {
        ...
        data["HD"]     = parseCorrectedRootTree(inputTree, 5, 6, 128);
    }
