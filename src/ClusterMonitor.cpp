...
void ClusterMonitor::WriteToNtuple(const ClusterCreator& clCreator, const std::string fileName, const feature_map features, bool pacific)
{
    ...
    for(const auto& clust : clCreator.getClusters())
    {
        ...
        if(pacific)
        {
            d_chargeWeightedMean = clust->GetChargeWeightedMeanPacific(1.5,2.5,4.0);
            d_sumCharge = clust->GetSumOfAdcValuesPacific(1.5,2.5,4.0);
        }
        else
        {
